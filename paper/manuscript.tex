...
\abstract{Automated decision-making systems are rarely deployed in isolation: in practice, organizations must combine outputs from multiple predictive models and reconcile them with strategic priorities, resource constraints, and scenario-dependent policies that resist crisp formalization. Existing cooperative AI frameworks address parts of this problem conceptually, and open-source fuzzy tools provide useful building blocks, but no reusable software currently operationalizes the full separation between predictive ensemble, contextual evaluation, and auditable prioritization. This paper addresses that gap with CADEMAS-ML, an open-source tool that implements the CADEMAS (Cooperative Automated Decision-Making Systems) framework for machine-learning-based case prioritization. Multiple predictive models act as cooperating ADM units; their performance-weighted outputs form an integrated risk score. A configurable fuzzy context layer, specified as JSON, evaluates scenario-dependent suitability. A single trade-off parameter $\lambda$ then combines both components into a transparent prioritization index that can be recomputed without touching the underlying models or context specifications. Two modules extend what was possible with our earlier conceptual and application-level work: \textit{Robustness} quantifies how rankings shift across the full $\lambda$ spectrum through rank trajectories and acceptability indices, making policy sensitivity explicit before any action is taken; and \textit{Explain} decomposes each priority score into its predictive and contextual contributions, traces the activated fuzzy rules, and generates a tier-based natural-language recommendation. The tool is validated on an employee-attrition scenario in which four departmental classifiers are combined under contrasting organizational contexts. The results show that identical predicted risks produce markedly different prioritizations under different context specifications, and that every ranking decision can be fully audited at the case level.}
...
\centering
\includegraphics[width=1.2\textwidth]{figures/robustness.pdf}
\end{adjustwidth}
